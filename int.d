...
if [[ $EUID -ne 0 ]]; then
    echo "${RED}Insufficient privileges: root access required.${RESET}"
    exit 1
fi

read -p "${CYAN}HOSTNAME:${RESET}" host_name
read -p "${CYAN}USERGROUP:${RESET}" user_group
read -p "${CYAN}USERGROUP_ID:${RESET}" user_grpid
read -p "${CYAN}MAILBASE:${RESET}" mail_base_rnd
mail_base="${mail_base_rnd%/}"
read -p "${CYAN}SSL_CERT:${RESET}" ssl_cert
read -p "${CYAN}SSL_KEY:${RESET}" ssl_key
...
echo -e "${YELLOW}setting up /etc/postfix/main.cf - ${GREEN}ok${RESET}"
...
echo -e "${YELLOW}setting up /etc/postfix/master.cf - ${GREEN}ok${RESET}"
...
echo -e "${YELLOW}setting up /etc/dovecot/dovecot.conf - ${GREEN}ok${RESET}"
...
echo -e "${YELLOW}setting up /etc/dovecot/conf.d/10-auth.conf - ${GREEN}ok${RESET}"
...
echo -e "${YELLOW}setting up /etc/dovecot/conf.d/10-logging.conf - ${GREEN}ok${RESET}"
...
echo -e "${YELLOW}setting up /etc/dovecot/conf.d/10-mail.conf - ${GREEN}ok${RESET}"
...
echo -e "${YELLOW}setting up /etc/dovecot/conf.d/10-ssl.conf - ${GREEN}ok${RESET}"
...
then
  ...
  then
    ...
    echo -e "\${YELLOW}Adding mail to postfix configuration...\${RESET}"
  fi
...
echo -e "${YELLOW}reloading services...${GREEN}success${RESET}"
